...
function M.detectors.lsp(buf)
  ...
  local clients = PowerVim.lsp.get_clients({ bufnr = buf })
...
function M.info()
  ...
  lines[#lines + 1] = "```"
  PowerVim.info(lines, { title = "PowerVim Roots" })
...
function M.setup()
  vim.api.nvim_create_user_command("PowerRoot", function()
    PowerVim.root.info()
  end, { desc = "PowerVim roots for the current buffer" })
  ...
  vim.api.nvim_create_autocmd({ "LspAttach", "BufWritePost", "DirChanged", "BufEnter" }, {
    group = vim.api.nvim_create_augroup("powervim_root_cache", { clear = true }),
...
function M.get(opts)
  ...
  return PowerVim.is_win() and ret:gsub("/", "\\") or ret
end
