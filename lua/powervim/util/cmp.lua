---@class powervim.util.cmp
local M = {}

---@alias powervim.util.cmp.Action fun():boolean?
---@type table<string, powervim.util.cmp.Action>
M.actions = {
  -- Native Snippets
...
function M.expand(snippet)
  ...
  -- See: https://github.com/PowerVim/PowerVim/issues/3199
...
function M.setup(opts)
  ...
  require("cmp.utils.snippet").parse = function(input)
    ...
    return PowerVim.cmp.snippet_preview(input)
  end
  ...
  cmp.event:on("confirm_done", function(event)
    if vim.tbl_contains(opts.auto_brackets or {}, vim.bo.filetype) then
      PowerVim.cmp.auto_brackets(event.entry)
    end
  end)
  cmp.event:on("menu_opened", function(event)
    PowerVim.cmp.add_missing_snippet_docs(event.window)
  end)
end
