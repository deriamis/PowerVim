...
function M.cmp_source(name, icon)
  icon = icon or GeekVim.config.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
  local started = false
...
function M.pretty_path(opts)
  ...
  return function(self)
    ...
    local root = GeekVim.root.get({ normalize = true })
    local cwd = GeekVim.root.cwd()
